...
std::string Day1::handlePart1Impl(const std::string& input) {
    std::ifstream file("input/2025/day1_test.txt");
    std::string line;
...
std::string Day1::handlePart2Impl(const std::string& input) {
    ...
    while (std::getline(file, line)) {
        ...
        if(movement > 0 && next_position < position) {
            count++;
        } // Consider landing and departing at 0 going Left, dont like it.
